function [ftsvm_struct,cloutput] = update_classify_fbtsvm_model(ftsvm_struct,trdata,trlabel,batch_size,Parameter,kobj,Testdata,Testlabel)
...
while p<inc_dsize
  ...
  [acc_2(iter),outclass,time(iter), fp(iter), fn(iter)]= classify_inc(ftsvm_struct,Testdata,Testlabel,kobj);

iter=iter+1;

end
cloutput.Accuracy=acc_2;
cloutput.time=time;
cloutput.fp=fp;
cloutput.fn=fn;
