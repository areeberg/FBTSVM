function  [ftsvm_struct,trdata,trlabel] = inc_bintrain_mem(Traindata,Trainlabel,Parameter,ftsvm_struct,data,label,Traindata_ori)
...
trainp_pos=find(Trainlabel==1); %Get the position of +1 samples


if isempty(trainp)==1
  warning('No +1 data');
else
  [S,delep]=createlinearSR(trainp,ftsvm_struct,'positive'); 
  if  isempty(S)==1
...
trainn_pos=find(Trainlabel==-1);

if isempty(trainn)==1
  warning('No -1 data');
else
  [R,delen]=createlinearSR(trainn,ftsvm_struct,'negative');  
  if  isempty(R)==1
  warning('After filtered there is no -1 data left');  
  else
     type(1,2)=1;
...
if isequal(type,[1 0])==1
   disp('Update +1 only');   
    ...
    trainp_ori=Traindata_ori((Trainlabel==1),:);
    trainp_ori(delep,:)=[];

    data_ori=[ftsvm_struct.oridata;trainp_ori];
    label_ori=[ftsvm_struct.orilabel;ones(size(trainp_ori,1),1)];
...
trainn_ori=Traindata_ori((Trainlabel==-1),:);


trainn_ori(delen,:)=[];
data_ori=[ftsvm_struct.oridata;trainn_ori];
label_ori=[ftsvm_struct.orilabel;-1*ones(size(trainn_ori,1),1)];    
Xpi=find(label==1);
Lpi=find(label==-1);
Xp=data(Xpi,:);
Xn=data(Lpi,:);
...
trainp_ori(delep,:)=[];
trainn_ori(delen,:)=[];
data_ori=[ftsvm_struct.oridata;trainp_ori;trainn_ori];
 label_ori=[ftsvm_struct.orilabel;ones(size(trainp_ori,1),1);-1*ones(size(trainn_ori,1),1)];
...
ftsvm_struct.oridata=data_ori;
ftsvm_struct.orilabel=label_ori;
end
